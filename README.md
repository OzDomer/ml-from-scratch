...
**What I built:** A `Value` class that wraps a number and lets you compose arithmetic into a tree-shaped graph. Each node remembers its parents and which operation created it. Each node also has a function that knows how to do one step of the chain rule: take the gradient sitting on this node and push it to its parents. After connecting each node to its parents during the forward pass, we can use those connections to "walk" backwards through the graph — getting gradient propagation from start to finish, even when it's a multi-step process.
